      ...
      <section id="beranda" className="relative overflow-hidden min-h-screen flex flex-col">
        ...
        <div className={`relative flex-1 flex max-w-6xl mx-auto w-full ${isMob ? "items-start justify-start pt-24 pb-12 px-6" : isDesk ? "items-center justify-center px-6" : "items-start justify-start pt-24 pb-12 px-6 md:items-center md:justify-center md:pt-0 md:pb-0 md:px-6"}`}>
          <div className={`max-w-3xl flex flex-col ${isMob ? "items-start text-left" : isDesk ? "items-center text-center" : "items-start text-left md:items-center md:text-center"}`}>
            <h1 className={`font-bold leading-[1.15] tracking-tight ${textPrimary} ${isMob ? "text-3xl mb-3" : isDesk ? "text-5xl lg:text-6xl mb-5" : "text-3xl md:text-5xl lg:text-6xl mb-3 md:mb-5"}`}>{hero.headline}</h1>
            <p className={`${textSecondary} leading-relaxed max-w-2xl ${isMob ? "text-[14px] mb-6" : isDesk ? "text-lg mb-8" : "text-[14px] md:text-lg mb-6 md:mb-8"}`}>{hero.subheadline}</p>
            <div className={`flex gap-2.5 ${isMob ? "flex-col w-full items-start" : isDesk ? "flex-row items-center justify-center gap-3" : "flex-col w-full items-start sm:flex-row sm:w-auto md:items-center md:justify-center gap-2.5 md:gap-3"}`}>
              <a href={waLink} target="_blank" rel="noopener noreferrer" className={`group text-white font-medium rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg ${isMob ? "px-5 py-3 text-[14px] w-full" : isDesk ? "px-8 py-3.5 text-[15px]" : "px-5 md:px-8 py-3 md:py-3.5 text-[14px] md:text-[15px] w-full sm:w-auto"}`} style={{ backgroundColor: pc, boxShadow: `0 10px 15px -3px ${pcBg20}` }}>
                <MessageCircle className="w-4 h-4" /> {hero.ctaText || "Hubungi via WhatsApp"} <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </a>
        ...
        <div className={`max-w-6xl mx-auto ${isMob ? "px-4" : isDesk ? "px-6" : "px-4 md:px-6"}`}>
          <div className={`${photos.length > 0 && !isMob ? "flex flex-col md:flex-row items-center gap-10" : "text-center max-w-4xl mx-auto"}`}>
            ...
            <div className={photos.length > 0 && !isMob ? "flex-1" : ""}>
              ...
              <h2 className={`font-bold tracking-tight mb-4 ${textPrimary} ${isMob ? "text-xl" : isDesk ? "text-3xl" : "text-xl md:text-3xl"}`}>{about.judul}</h2>
              <p className={`${textSecondary} leading-relaxed ${isMob ? "text-[13px] mb-8" : isDesk ? "text-[15px] mb-10" : "text-[13px] md:text-[15px] mb-8 md:mb-10"}`}>{about.deskripsi}</p>
                  ...
                  return (
                    <div key={i} className={`flex flex-col items-center gap-2 rounded-xl border ${cardBg} ${cardBorder} ${isMob ? "p-3" : isDesk ? "p-5" : "p-3 md:p-5"}`}>
                      ...
                      <div className="text-center">
                        <p className={`font-semibold ${textPrimary} ${isMob ? "text-[12px]" : "text-[13px]"}`}>{k}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            {layanan.map((l, i) => {
              ...
              return (
                <div key={i}
                  ...
                  <h3 className={`font-semibold mb-1.5 ${textPrimary} ${isMob ? "text-[14px]" : "text-[15px]"}`}>{l.nama}</h3>
                  <p className={`${textTertiary} leading-relaxed ${isMob ? "text-[12px]" : "text-[13px]"}`}>{l.deskripsi}</p>
                </div>
              );
            })}
          </div>
              ...
              return (
                <div key={i} className={`${cardBg} border ${cardBorder} rounded-2xl hover:shadow-lg transition-shadow overflow-hidden`}>
                  ...
                  <div className={isMob ? "p-5" : "p-6"}>
                    ...
                    <p className={`${textSecondary} leading-relaxed mb-4 ${isMob ? "text-[12px]" : "text-[13px]"}`}>&quot;{t.teks}&quot;</p>
                    <div className="flex items-center gap-1 mb-3">
                      {Array.from({ length: 5 }).map((_, s) => (
                        <Star key={s} className={`w-3.5 h-3.5 ${s < 5 ? "text-amber-400 fill-amber-400" : isDark ? "text-zinc-700" : "text-zinc-200"}`} />
                      ))}
                    </div>
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className={`font-semibold ${textPrimary} ${isMob ? "text-[12px]" : "text-[13px]"}`}>{t.nama}</p>
                        <p className={`${textTertiary} ${isMob ? "text-[10px]" : "text-[11px]"}`}>{t.peran}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className={`max-w-6xl mx-auto ${isMob ? "px-4" : isDesk ? "px-6" : "px-4 md:px-6"}`}>
          <div className={`relative bg-zinc-900 rounded-2xl overflow-hidden ${isMob ? "p-8" : isDesk ? "p-14" : "p-8 md:p-14"}`}>
            ...
            <div className="relative text-center max-w-2xl mx-auto">
              <h2 className={`font-bold text-white tracking-tight ${isMob ? "text-xl mb-3" : isDesk ? "text-3xl mb-4" : "text-xl md:text-3xl mb-3 md:mb-4"}`}>{footer.tagline || "Siap untuk Memulai?"}</h2>
              ...
              <a href={waLink} target="_blank" rel="noopener noreferrer"
                ...
                <MessageCircle className="w-4 h-4" /> {footer.ctaText || "Chat WhatsApp Sekarang"} <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
              </a>
            </div>
          </div>
        </div>
        ...
        <div className={`max-w-6xl mx-auto ${isMob ? "px-4" : isDesk ? "px-6" : "px-4 md:px-6"}`}>
          <div className={`grid gap-8 ${isMob ? "grid-cols-1" : isDesk ? "grid-cols-3 gap-10" : "grid-cols-1 md:grid-cols-3 md:gap-10"}`}>
            ...
            <div>
              ...
              <p className={`${textTertiary} text-[12px] leading-relaxed max-w-xs`}>{about.deskripsi.substring(0, 120)}...</p>
            </div>
